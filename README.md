...
현재 설정의 URL은 어댑터 연결을 위한 예시입니다. 하나은행 URL은 환율 페이지이고 KB URL은 404 페이지가 될 수 있으므로, 실제 지점 검색 URL로 교체한 뒤 `enabled`를 `true`로 설정해야 합니다. 지점 검색 결과가 JavaScript로 렌더링되면 해당 은행 어댑터에 브라우저 수집 단계를 추가해야 합니다.
